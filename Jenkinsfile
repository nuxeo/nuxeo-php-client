      ...
      try {
        ...
        dockerImage.inside {
          ...
          stage('dependencies vulnerability check') {
            sh 'php vendor/bin/security-checker security:check --end-point=http://security.symfony.com/check_lock'
          }
          stage('unit tests') {
            sh 'php vendor/bin/phpunit --exclude-group server'
          }
        }
        dockerImage.withRun("-v ${env.WORKSPACE}:${env.WORKSPACE} -w ${env.WORKSPACE}/ftests --link ${ctnrId}:nuxeo", 'tail -f /dev/null') { c ->
          stage('ftests') {
            dir('ftests') {
              ...
              sh """#!/usr/bin/env bash
                chmod +x bin/php
                curl -sS https://getcomposer.org/installer | docker exec -i -u jenkins ${c.id} php
